...
const WIDGET_DICT = {
    ...
    pending_tasks: { icon: 'âœ…', label: 'Pending Tasks (Mini)' },
    todays_events: { icon: 'ðŸ“…', label: 'Today\'s Events (Mini)' },
    net_money: { icon: 'ðŸ’°', label: 'Net Money (Mini)' },
    active_counters: { icon: 'ðŸ”¢', label: 'Active Counters (Mini)' },
    alarms: { icon: 'â°', label: 'Active Alarms (Mini)' },
    sleep: { icon: 'ðŸ˜´', label: 'Last Night\'s Sleep (Mini)' },
    roadmap: { icon: 'ðŸ—ºï¸', label: 'Active Roadmap (Mini)' },
    specific_account: { icon: 'ðŸ’³', label: 'Pinned Account (Mini)' },
    specific_counter: { icon: 'ðŸ”¢', label: 'Pinned Counter (Mini)' },
    specific_note: { icon: 'ðŸ“', label: 'Pinned Note (Mini)' },

    // FULL WIDGETS (Full Width)
    redzone: { icon: 'ðŸš¨', label: 'Urgent Deadlines (Full)' },
    schedule: { icon: 'ðŸ“…', label: 'Today\'s Schedule (Full)' },
    tasks: { icon: 'ðŸ“‹', label: 'Upcoming Tasks (Full)' },
    classes: { icon: 'ðŸŽ“', label: 'Today\'s Classes (Full)' }
};

// NEW: Master Dictionary of all App Modules
const NAV_MODULES = {
    dash: { icon: 'ðŸ ', label: 'Home' },
    planner: { icon: 'ðŸ“…', label: 'Planner' },
    tasks: { icon: 'âœ…', label: 'Tasks' },
    counter: { icon: 'ðŸ”¢', label: 'Counter' },
    money: { icon: 'ðŸ’°', label: 'Money' },
    alarms: { icon: 'â°', label: 'Alarms' },
    roadmap: { icon: 'ðŸ—ºï¸', label: 'Roadmap' },
    attendance: { icon: 'ðŸ“Š', label: 'Attend' },
    academic: { icon: 'ðŸŽ“', label: 'School' },
    vault: { icon: 'ðŸ”’', label: 'Vault' },
    expenses: { icon: 'ðŸ’¸', label: 'Expenses' },
    notes: { icon: 'ðŸ“', label: 'Notes' },
    sleep: { icon: 'ðŸ˜´', label: 'Sleep' },
...
async function processSyncQueue() {
    ...
    save();
    if (ok > 0) { toast(`â˜ï¸ Synced ${ok} item${ok > 1 ? 's' : ''} to cloud!`); hideSyncBadge(); load(); }
...
function showSyncBadge() {
    ...
    if (!b) {
        b = document.createElement('div'); b.id = 'syncBadge'; b.title = 'Pending offline changes â€“ will sync when online'; b.innerHTML = 'ðŸ“¶ <span id="syncCount"></span>';
...
function showSyncDetails() {
    if (!STATE.syncQueue || STATE.syncQueue.length === 0) return toast('All data is synced! â˜ï¸');
...
async function load() {
    ...
    } catch (err) { console.log('Offline mode â€” using cached data', err); if ((STATE.syncQueue || []).length > 0) showSyncBadge(); }
}
...
function updateGreeting() {
    const h = new Date().getHours(); const g = h < 12 ? 'Good morning ðŸ‘‹' : h < 17 ? 'Good afternoon â˜€ï¸' : 'Good evening ðŸŒ™';
    document.getElementById('greeting').textContent = g;
    const days = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
    document.getElementById('greetingSub').textContent = `${days[new Date().getDay()]} â€” let's get things done`;
...
function renderDashboard() {
        ...
        try {
            switch (widget) {
                ...
                case 'pending_tasks': {
                    ...
                    wHtml = `<div class="stat-card"><div class="stat-icon">âœ…</div><div class="stat-num">${activeTasks}</div><div class="stat-label">Pending Tasks</div></div>`;
                    break;
                }
                case 'todays_events': {
                    wHtml = `<div class="stat-card"><div class="stat-icon">ðŸ“…</div><div class="stat-num">${STATE.plans.filter(p => isEventOnDate(p, todayStr)).length}</div><div class="stat-label">Today's Events</div></div>`;
                    break;
                }
                case 'net_money': {
                    ...
                    wHtml = `<div class="stat-card"><div class="stat-icon">ðŸ’°</div><div class="stat-num" style="color:${netColor}">${netMoney >= 0 ? '+' : '-'}$${Math.abs(netMoney).toFixed(0)}</div><div class="stat-label">Net Money</div></div>`;
                    break;
                }
                case 'active_counters': {
                    wHtml = `<div class="stat-card"><div class="stat-icon">ðŸ”¢</div><div class="stat-num">${STATE.counters.length}</div><div class="stat-label">Active Counters</div></div>`;
                    break;
                }
                case 'alarms': {
                    const activeAlarms = STATE.alarms.filter(a => a.enabled && a.time).sort((a, b) => a.time > b.time ? 1 : -1);
                    if (activeAlarms.length === 0) wHtml = `<div class="stat-card"><div class="stat-icon">â°</div><div class="stat-num" style="font-size:20px; padding:4px 0;">Off</div><div class="stat-label">Next Alarm</div></div>`;
                    else wHtml = `<div class="stat-card"><div class="stat-icon">â°</div><div class="stat-num" style="font-size:22px; padding:2px 0;">${formatTime(activeAlarms[0].time)}</div><div class="stat-label">${activeAlarms[0].label || 'Next Alarm'}</div></div>`;
                    break;
                }
                case 'sleep': {
                    if (STATE.sleepLogs.length === 0) wHtml = `<div class="stat-card"><div class="stat-icon">ðŸ˜´</div><div class="stat-num" style="font-size:20px; padding:4px 0;">No Data</div><div class="stat-label">Last Night</div></div>`;
                    else {
                        const latest = [...STATE.sleepLogs].sort((a, b) => (a.date || '').localeCompare(b.date || '')).pop();
                        wHtml = `<div class="stat-card"><div class="stat-icon">ðŸ˜´</div><div class="stat-num" style="color:var(--accent2);">${(latest.durationMins / 60).toFixed(1)}h</div><div class="stat-label">Last Night</div></div>`;
                    }
                    break;
                }
                case 'roadmap': {
                    if (STATE.roadmaps.length === 0) wHtml = `<div class="stat-card"><div class="stat-icon">ðŸ—ºï¸</div><div class="stat-num" style="font-size:20px; padding:4px 0;">None</div><div class="stat-label">Active Roadmap</div></div>`;
                    else {
                        ...
                        wHtml = `<div class="stat-card"><div class="stat-icon">ðŸ—ºï¸</div><div class="stat-num" style="font-size:24px; padding:2px 0;">${bestPct.toFixed(0)}%</div><div class="stat-label" style="white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${bestR.title}</div></div>`;
                    }
                    break;
                }
                case 'specific_account': {
                    if (!STATE.dashConfig.accountId) wHtml = `<div class="stat-card"><div class="stat-icon">ðŸ’³</div><select class="input" style="padding:4px; font-size:11px; margin-top:4px;" onchange="STATE.dashConfig.accountId = this.value; save(); renderDashboard();"><option value="">Select Account</option>${STATE.accounts.map(a => `<option value="${a.id}">${a.name}</option>`).join('')}</select></div>`;
                    else {
                        const acc = STATE.accounts.find(a => a.id == STATE.dashConfig.accountId);
                        if (!acc) wHtml = `<div class="stat-card"><div class="stat-icon">ðŸ’³</div><div class="stat-num" style="font-size:16px; padding:8px 0; color:var(--red);">Deleted</div><div class="stat-label" onclick="STATE.dashConfig.accountId = null; save(); renderDashboard();" style="cursor:pointer; text-decoration:underline;">Reset</div></div>`;
                        else {
                            const bal = typeof getAccountBalance === 'function' ? getAccountBalance(acc.id) : 0;
                            wHtml = `<div class="stat-card" style="position:relative;"><span style="position:absolute; top:8px; right:8px; font-size:10px; opacity:0.5; cursor:pointer;" onclick="STATE.dashConfig.accountId = null; save(); renderDashboard();">âš™ï¸</span><div class="stat-icon">ðŸ’³</div><div class="stat-num" style="font-size:20px; padding:4px 0; color:${bal >= 0 ? 'var(--accent2)' : 'var(--red)'};">$${bal.toFixed(0)}</div><div class="stat-label" style="white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${acc.name}</div></div>`;
                        }
                    }
                    break;
                }
                case 'specific_counter': {
                    if (!STATE.dashConfig.counterId) wHtml = `<div class="stat-card"><div class="stat-icon">ðŸ”¢</div><select class="input" style="padding:4px; font-size:11px; margin-top:4px;" onchange="STATE.dashConfig.counterId = this.value; save(); renderDashboard();"><option value="">Select Counter</option>${STATE.counters.map(c => `<option value="${c.id}">${c.name}</option>`).join('')}</select></div>`;
                    else {
                        const c = STATE.counters.find(x => x.id == STATE.dashConfig.counterId);
                        if (!c) wHtml = `<div class="stat-card"><div class="stat-icon">ðŸ”¢</div><div class="stat-num" style="font-size:16px; padding:8px 0; color:var(--red);">Deleted</div><div class="stat-label" onclick="STATE.dashConfig.counterId = null; save(); renderDashboard();" style="cursor:pointer; text-decoration:underline;">Reset</div></div>`;
                        else {
                            wHtml = `<div class="stat-card" style="position:relative;"><span style="position:absolute; top:8px; right:8px; font-size:10px; opacity:0.5; cursor:pointer;" onclick="STATE.dashConfig.counterId = null; save(); renderDashboard();">âš™ï¸</span><div class="stat-icon" style="color:${c.color};">ðŸ”¢</div><div class="stat-num" style="font-size:24px; padding:2px 0;">${c.value}</div><div style="display:flex; justify-content:center; gap:4px; margin-top:4px;"><button class="btn-secondary" style="padding:2px 8px; font-size:12px;" onclick="adjustCounter(${c.id},-1); setTimeout(renderDashboard, 50)">-</button><button class="btn-secondary" style="padding:2px 8px; font-size:12px;" onclick="adjustCounter(${c.id},1); setTimeout(renderDashboard, 50)">+</button></div><div class="stat-label" style="margin-top:6px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${c.name}</div></div>`;
                        }
                    }
                    break;
                }
                case 'specific_note': {
                    if (!STATE.dashConfig.noteId) wHtml = `<div class="stat-card"><div class="stat-icon">ðŸ“</div><select class="input" style="padding:4px; font-size:11px; margin-top:4px;" onchange="STATE.dashConfig.noteId = this.value; save(); renderDashboard();"><option value="">Select Note</option>${STATE.notes.map(n => `<option value="${n.id}">${n.title || 'Untitled'}</option>`).join('')}</select></div>`;
                    else {
                        const n = STATE.notes.find(x => x.id == STATE.dashConfig.noteId);
                        if (!n) wHtml = `<div class="stat-card"><div class="stat-icon">ðŸ“</div><div class="stat-num" style="font-size:16px; padding:8px 0; color:var(--red);">Deleted</div><div class="stat-label" onclick="STATE.dashConfig.noteId = null; save(); renderDashboard();" style="cursor:pointer; text-decoration:underline;">Reset</div></div>`;
                        else {
                            let rawText = n.body || '';
                ...
                /* ------------------ FULL WIDGETS ------------------ */
                case 'redzone': {
                    let uHtml = `<div class="card" style="margin-bottom:12px;"><div class="section-header" style="margin-bottom:10px;"><div class="section-title" style="color:var(--red);">ðŸš¨ Urgent Deadlines</div></div>`;
                ...
                case 'schedule': {
                    let sHtml = `<div class="card" style="margin-bottom:12px;"><div class="section-header" style="margin-bottom:10px;"><div class="section-title">ðŸ“… Today's Schedule</div></div>`;
                ...
                case 'tasks': {
                    let tHtml = `<div class="card" style="margin-bottom:12px;"><div class="section-header" style="margin-bottom:10px;"><div class="section-title">ðŸ“‹ Upcoming Tasks</div></div>`;
                    ...
                    else tHtml += upcoming.map(t => `<div class="task-item" style="margin-bottom:8px; cursor:default;"><div class="priority-dot p${t.priority}" style="margin-top:6px"></div><div class="task-body"><div class="task-title">${t.title}</div><div class="task-due">${t.category} ${t.due ? 'Â· ' + fmtDisplay(t.due) : ''}</div></div></div>`).join('');
                    tHtml += `</div>`;
                    wHtml = tHtml;
                    break;
                }
                case 'classes': {
                    let cHtml = `<div class="card" style="margin-bottom:12px;"><div class="section-header" style="margin-bottom:10px;"><div class="section-title">ðŸŽ“ Today's Classes</div></div>`;
    ...
    if (html === '') {
        html = '<div class="empty-state"><div class="empty-icon">âœ¨</div><p>Your dashboard is clear.<br>Add widgets from Settings to customize this space.</p></div>';
    }
    ...
    STATE.dashWidgets.forEach((key, index) => {
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; background:var(--surface2); padding:10px 14px; margin-bottom:8px; border-radius:10px; border:1px solid var(--border);">
            ...
            <div style="display:flex; gap:6px;">
                <button class="btn-secondary" style="padding:6px 10px; font-size:14px;" onclick="moveDashWidget(${index}, -1)" ${index === 0 ? 'disabled style="opacity:0.3"' : ''}>â–²</button>
                <button class="btn-secondary" style="padding:6px 10px; font-size:14px;" onclick="moveDashWidget(${index}, 1)" ${index === STATE.dashWidgets.length - 1 ? 'disabled style="opacity:0.3"' : ''}>â–¼</button>
...
function renderVault() {
    ...
    if (!currentVaultFolderId) {
        breadcrumb.innerHTML = `<span style="color:var(--text); font-weight:700;">ðŸ  Root Directory</span>`;
    } else {
        const currentF = (STATE.vaultFolders || []).find(f => f.id == currentVaultFolderId);
        breadcrumb.innerHTML = `
            <span style="cursor: pointer; color: var(--accent);" onclick="navigateVaultFolder(null)">ðŸ  Root</span>
            <span>/</span>
            <span style="color: var(--text); font-weight: 700;">ðŸ“ ${escapeHtml(currentF ? currentF.name : 'Folder')}</span>
        `;
    }
    ...
    if (!currentVaultFolderId) {
        ...
        if (!STATE.vaultFolders || STATE.vaultFolders.length === 0) {
            foldersContainer.innerHTML = '';
        } else {
            foldersContainer.innerHTML = STATE.vaultFolders.map(f => {
                ...
                <div onclick="navigateVaultFolder(${f.id})" style="background:var(--surface); border:1px solid var(--border); border-radius:var(--radius-sm); padding:14px; cursor:pointer; position:relative; transition:0.2s;">
                    <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                        ...
                        <span onclick="deleteVaultFolder(event, ${f.id})" style="color:var(--red); font-size:14px; cursor:pointer; padding:2px;" title="Delete Folder">ðŸ—‘</span>
                    </div>
                    ...
                    <div style="font-size:11px; color:var(--text3); margin-top:4px;">${count} file${count !== 1 ? 's' : ''}</div>
                </div>`;
            }).join('');
        }
    } else {
    ...
    filesContainer.innerHTML = activeFiles.map(f => {
        ...
        let icon = 'ðŸ“„';
        if (['jpg', 'jpeg', 'png', 'webp', 'gif', 'svg'].includes(ext)) icon = 'ðŸ–¼ï¸';
        else if (ext === 'pdf') icon = 'ðŸ“•';
        else if (['ppt', 'pptx'].includes(ext)) icon = 'ðŸ“Š';
        else if (['js', 'css', 'html', 'c', 'cpp', 'py', 'java', 'json', 'sql', 'ts'].includes(ext)) icon = 'ðŸ’»';
        else if (ext === 'txt' || ext === 'md') icon = 'ðŸ“';
        ...
        <div class="card-sm" style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px; background:var(--surface);">
            ...
            <div style="display:flex; gap:8px; align-items:center;">
                ...
                <button onclick="openMoveModal(${f.id})" class="btn-secondary" style="padding:4px 8px; font-size:11px;" title="Move File">ðŸ“‚</button>
                <button onclick="deleteVaultFile(${f.id}, '${f.filepath}')" style="background:none; border:none; color:var(--red); font-size:15px; cursor:pointer; padding:4px;">ðŸ—‘</button>
            </div>
        </div>`;
    }).join('');
}
...
async function saveVaultFolder() {
    ...
    try {
        ...
        const { data, error } = await supabaseClient.from('vault_folders').insert([{ name, user_id: userId }]).select();
        ...
        toast('Folder created! ðŸ“');
    } catch (err) {
        console.error('Folder creation error:', err);
        toast('Failed to create folder in database: ' + (err.message || ''));
    }
}
...
function openMoveModal(fileId) {
    ...
    select.innerHTML = '<option value="">ðŸ  Root (No Folder)</option>' +
        (STATE.vaultFolders || []).map(f => `<option value="${f.id}">ðŸ“ ${escapeHtml(f.name)}</option>`).join('');
...
async function uploadFile(files) {
    ...
    try {
        ...
        toast('Uploaded & Saved to Vault! ðŸ”’');
        renderVault();
    } catch (err) {
        if (dz) dz.style.borderColor = 'var(--accent)';
...
async function previewVaultFile(filename, url) {
    ...
    if (['jpg', 'jpeg', 'png', 'webp', 'gif', 'svg'].includes(ext)) {
        ...
        bodyEl.innerHTML = `<div style="color:var(--text2); font-size:13px;">Loading source file... â³</div>`;
...
function goToToday() { const today = new Date(); const str = fmtDate(today); calCurrentDate = today; STATE.selectedDate = str; document.getElementById('goToDateInput').value = ''; renderCalendar(); renderPlanner(); toast('Jumped to today ðŸ“…'); }
...
function savePlan() {
    ...
    renderCalendar(); renderPlanner(); renderDashboard(); closeModal('plannerModal'); save(); toast('Event added ðŸ“…');
...
function deletePlan(e, id) { if (e) e.stopPropagation(); if (!confirm('Are you sure you want to delete this event series?')) return; STATE.plans = STATE.plans.filter(x => x.id !== id); renderCalendar(); renderPlanner(); renderDashboard(); save(); toast('Deleted ðŸ—‘ï¸'); ofetch('delete_plan.php', { id }); }
function renderPlanner() {
    ...
    if (selPlans.length === 0) { html += `<div style="text-align:center; color:var(--text3); font-size:13px; margin-bottom:24px;">No events scheduled for this day.</div>`; } else {
        html += selPlans.map(p => `<div class="time-slot"><div class="time-label">${formatTime(p.time)}</div><div class="time-line" style="background:${p.color}"></div><div class="time-events" style="flex:1"><div class="event-block ${p.completed ? 'done' : ''}" style="border-color:${p.color}" onclick="togglePlan(event, ${p.id})"><div class="event-title">${p.title}</div>${p.desc ? `<div class="event-desc">${p.desc}</div>` : ''}<div style="display:flex;justify-content:space-between;margin-top:4px"><span style="font-size:10px;color:var(--text3)">${p.completed ? 'âœ“ Done' : (p.recurrence && p.recurrence !== 'none' ? 'ðŸ” ' + p.recurrence : 'â° ' + p.time)}</span><span onclick="deletePlan(event, ${p.id})" style="font-size:16px;color:var(--text3);cursor:pointer;padding:4px">ðŸ—‘</span></div></div></div></div>`).join('');
    }
    ...
    if (allUpcoming.length === 0) { html += `<div style="text-align:center; color:var(--text3); font-size:13px; padding-bottom:20px;">No upcoming events.</div>`; } else {
        ...
        allUpcoming.forEach(e => {
            ...
            if (e.isAcad) { html += `<div class="event-block" style="border-color:var(--accent3); margin-bottom:8px; cursor:pointer;" onclick="navTo('academic'); openAcademicModalById(${e.id})"><div style="display:flex; justify-content:space-between; align-items:center;"><div style="font-size:10px; color:var(--accent3); font-weight:600; text-transform:uppercase;">ðŸŽ“ ${e.type}</div><span onclick="delAcademic(event, ${e.id})" style="font-size:16px;color:var(--text3);cursor:pointer;padding:4px;">ðŸ—‘</span></div><div class="event-title">${e.subject}</div>${e.topic ? `<div class="event-desc">${e.topic}</div>` : ''}</div>`; }
            else { html += `<div class="event-block ${e.completed ? 'done' : ''}" style="border-color:${e.color}; margin-bottom:8px;" onclick="togglePlan(event, ${e.id})"><div style="display:flex; justify-content:space-between; align-items:center;"><div class="event-title">${e.title}</div><div style="display:flex; gap:8px; align-items:center;"><div style="font-size:10px; color:var(--text3)">${e.recurrence && e.recurrence !== 'none' ? 'ðŸ”' : 'â°'} ${formatTime(e.time)}</div><span onclick="deletePlan(event, ${e.id})" style="font-size:16px;color:var(--text3);cursor:pointer;padding:4px;">ðŸ—‘</span></div></div></div>`; }
        });
    }
    el.innerHTML = html;
}
...
function saveTask() {
    ...
    if (editId) {
        ...
        renderTasks(); renderDashboard(); closeModal('taskModal'); save(); ofetch('update_task_details.php', taskData, () => toast('Task updated! âœ…'));
    } else {
        const tempId = Date.now(); taskData.id = tempId; taskData.completed = false; STATE.tasks.unshift(taskData);
        renderTasks(); renderDashboard(); closeModal('taskModal'); save(); toast('Saved! âœ…');
...
function toggleTask(e, id) { if (e) e.stopPropagation(); const t = STATE.tasks.find(x => x.id === id); if (!t) return; t.completed = !t.completed; renderTasks(); renderDashboard(); save(); toast(t.completed ? 'Task done! ðŸŽ‰' : 'Task reopened'); ofetch('update_task.php', { id, completed: t.completed }); }
function deleteTask(e, id) { if (e) e.stopPropagation(); if (!confirm('Are you sure you want to delete this task?')) return; STATE.tasks = STATE.tasks.filter(t => t.id !== id); renderTasks(); renderDashboard(); save(); toast('Task deleted ðŸ—‘ï¸'); ofetch('delete_task.php', { id }); }
...
function renderTasks() {
    ...
    const el = document.getElementById('taskList'); if (tasks.length === 0) return el.innerHTML = '<div class="empty-state"><div class="empty-icon">âœ…</div><p>No tasks here</p></div>';
    el.innerHTML = tasks.map(t => `<div class="task-item ${t.completed ? 'done' : ''}" onclick="openTaskModalById(${t.id})" style="cursor:pointer;"><div class="task-check ${t.completed ? 'checked' : ''}" onclick="toggleTask(event, ${t.id})"></div><div class="task-body"><div class="task-title">${t.title}</div><div class="task-meta"><span class="priority-dot p${t.priority}"></span><span class="pill pill-accent" style="font-size:10px;padding:2px 7px">${t.category}</span>${t.due ? `<span class="task-due">ðŸ“… ${fmtDisplay(t.due)}</span>` : ''} ${t.priority === 2 ? '<span style="font-size:11px;color:var(--red)">ðŸ”´ High</span>' : ''}</div></div><div class="task-delete" onclick="deleteTask(event, ${t.id})">ðŸ—‘</div></div>`).join('');
}
...
function deleteCounter(e, id) { if (e) e.stopPropagation(); if (!confirm('Are you sure you want to delete this counter?')) return; STATE.counters = STATE.counters.filter(x => x.id !== id); renderCounters(); save(); toast('Counter deleted ðŸ—‘ï¸'); ofetch('delete_counter.php', { id }); }
...
function renderCounters() {
    ...
    if (STATE.counters.length === 0) return el.innerHTML = '<div class="empty-state" style="grid-column:span 2"><div class="empty-icon">ðŸ”¢</div><p>Create your first counter</p></div>';
    ...
    el.innerHTML = STATE.counters.map(c => {
        ...
        <div class="counter-card">
            ...
            <div class="counter-controls">
                <button class="c-btn c-btn-minus" onclick="adjustCounter(${c.id},-1)">âˆ’</button>
                <button class="c-btn c-btn-reset" onclick="resetCounter(${c.id})">â†º</button>
            ...
            <div style="text-align:right;margin-top:8px">
                <span onclick="deleteCounter(event, ${c.id})" style="font-size:16px;color:var(--text3);cursor:pointer;padding:4px;">ðŸ—‘</span>
            </div>
        </div>`;
    }).join('');
}
...
// MONEY
// ============================================================
function saveMoney() { const person = document.getElementById('moneyPerson').value.trim(); const amount = parseFloat(document.getElementById('moneyAmount').value); if (!person || !amount) return toast('Fill required fields'); const mData = { person, amount, type: document.getElementById('moneyType').value, note: document.getElementById('moneyNote').value, due: document.getElementById('moneyDue').value }; const tempId = Date.now(); mData.id = tempId; mData.settled = false; STATE.money.push(mData); renderMoney(); renderDashboard(); closeModal('moneyModal'); save(); toast('Saved ðŸ’°'); ofetch('add_money.php', mData, d => { const m = STATE.money.find(x => x.id === tempId); if (m) m.id = d.id; renderMoney(); renderDashboard(); save(); }); }
function settleMoney(e, id) { if (e) e.stopPropagation(); const m = STATE.money.find(x => x.id === id); if (!m) return; m.settled = true; renderMoney(); save(); toast('Settled âœ“'); ofetch('update_money.php', { id }); }
function deleteMoney(e, id) { if (e) e.stopPropagation(); if (!confirm('Are you sure you want to delete this money record?')) return; STATE.money = STATE.money.filter(x => x.id !== id); renderMoney(); save(); toast('Deleted ðŸ—‘ï¸'); ofetch('delete_money.php', { id }); }
...
function renderMoney() { let records = [...STATE.money]; if (STATE.moneyFilter === 'lent') records = records.filter(m => m.type === 'lent'); else if (STATE.moneyFilter === 'borrowed') records = records.filter(m => m.type === 'borrowed'); else if (STATE.moneyFilter === 'pending') records = records.filter(m => !m.settled); else if (STATE.moneyFilter === 'settled') records = records.filter(m => m.settled); const lentTotal = STATE.money.filter(m => m.type === 'lent' && !m.settled).reduce((s, m) => s + parseFloat(m.amount || 0), 0); const owedTotal = STATE.money.filter(m => m.type === 'borrowed' && !m.settled).reduce((s, m) => s + parseFloat(m.amount || 0), 0); document.getElementById('totalLent').textContent = '$' + lentTotal.toFixed(2); document.getElementById('totalOwed').textContent = '$' + owedTotal.toFixed(2); const el = document.getElementById('moneyList'); if (records.length === 0) return el.innerHTML = '<div class="empty-state"><div class="empty-icon">ðŸ’°</div><p>No records found</p></div>'; el.innerHTML = records.map(m => `<div class="money-item ${m.settled ? 'money-settled' : ''}"><div class="money-avatar ${m.type}">${m.person[0].toUpperCase()}</div><div class="money-info"><div class="money-name">${m.person} ${m.settled ? '<span class="pill pill-green" style="font-size:9px">Settled</span>' : ''}</div><div class="money-note">${m.note || (m.type === 'lent' ? 'You lent' : 'You borrowed')} ${m.due ? 'Â· Due ' + fmtDisplay(m.due) : ''}</div></div><div style="text-align:right"><div class="money-amount ${m.type}">${m.type === 'lent' ? '+' : '-'}$${parseFloat(m.amount).toFixed(2)}</div>${!m.settled ? `<button class="settle-btn" onclick="settleMoney(event, ${m.id})">Settle</button>` : ''}<div onclick="deleteMoney(event, ${m.id})" style="font-size:16px;color:var(--text3);cursor:pointer;margin-top:4px;padding:4px;">ðŸ—‘</div></div></div>`).join(''); }

// ============================================================
// ALARMS
// ============================================================
function saveAlarm() { const time = document.getElementById('alarmTime').value; if (!time) return toast('Please set a time'); const days = [...document.querySelectorAll('.day-btn.selected')].map(b => b.dataset.day).join(''); const [h, m] = time.split(':').map(Number); const aData = { time, hour: h, minute: m, label: document.getElementById('alarmLabel').value || 'Alarm', days }; const tempId = Date.now(); aData.id = tempId; aData.enabled = true; STATE.alarms.push(aData); renderAlarms(); closeModal('alarmModal'); save(); toast('Alarm set â°'); ofetch('add_alarm.php', aData, d => { const a = STATE.alarms.find(x => x.id === tempId); if (a) a.id = d.id; renderAlarms(); renderDashboard(); save(); }); }
...
function deleteAlarm(e, id) { if (e) e.stopPropagation(); if (!confirm('Are you sure you want to delete this alarm?')) return; STATE.alarms = STATE.alarms.filter(x => x.id !== id); renderAlarms(); renderDashboard(); save(); toast('Alarm deleted ðŸ—‘ï¸'); ofetch('delete_alarm.php', { id }); }
...
function renderAlarms() { const el = document.getElementById('alarmList'); if (STATE.alarms.length === 0) return el.innerHTML = '<div class="empty-state"><div class="empty-icon">â°</div><p>No alarms set</p></div>'; el.innerHTML = STATE.alarms.map(a => { const [h, m] = a.time.split(':').map(Number); const ampm = h >= 12 ? 'PM' : 'AM'; const displayH = h > 12 ? h - 12 : h === 0 ? 12 : h; const dayNames = ['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa']; const daysStr = a.days ? a.days.split('').map(d => dayNames[parseInt(d)]).join(' ') : 'Once'; return `<div class="alarm-item ${!a.enabled ? 'money-settled' : ''}"><div style="flex:1"><div class="alarm-time">${String(displayH).padStart(2, '0')}:${String(m).padStart(2, '0')}<span class="alarm-time-ampm">${ampm}</span></div><div class="alarm-label">${a.label}</div><div class="alarm-days">${daysStr}</div></div><label class="alarm-toggle"><input type="checkbox" ${a.enabled ? 'checked' : ''} onchange="toggleAlarm(event, ${a.id})"><span class="toggle-slider"></span></label><div class="alarm-delete" onclick="deleteAlarm(event, ${a.id})">ðŸ—‘</div></div>`; }).join(''); }
...
// ROADMAPS
// ============================================================
function saveRoadmap() { const title = document.getElementById('roadmapTitle').value.trim(); if (!title) return toast('Please enter a title'); const rData = { title, desc: document.getElementById('roadmapDesc').value, category: document.getElementById('roadmapCategory').value, target: document.getElementById('roadmapTarget').value, color: selectedColors.roadmap }; const tempId = Date.now(); rData.id = tempId; rData.pendingSync = true; STATE.roadmaps.push(rData); renderRoadmaps(); closeModal('roadmapModal'); save(); toast('Roadmap created ðŸ—ºï¸'); ofetch('add_roadmap.php', rData, d => { const r = STATE.roadmaps.find(x => x.id === tempId); if (r) { r.id = d.id; r.pendingSync = false; } renderRoadmaps(); renderDashboard(); save(); }); }
function deleteRoadmap(e, id) { if (e) e.stopPropagation(); if (!confirm('Are you sure you want to delete this entire roadmap? All steps will be lost.')) return; STATE.roadmaps = STATE.roadmaps.filter(r => r.id !== id); STATE.steps = STATE.steps.filter(s => s.roadmapId !== id); document.getElementById('roadmapDetail-view').style.display = 'none'; document.getElementById('roadmapList-view').style.display = 'block'; renderRoadmaps(); save(); toast('Roadmap Deleted! ðŸ—‘ï¸'); ofetch('delete_roadmap.php', { id }); }
function saveStep() { const title = document.getElementById('stepTitle').value.trim(); if (!title) return toast('Please enter a title'); const rid = parseInt(document.getElementById('stepModal').dataset.roadmapId); const order = STATE.steps.filter(s => s.roadmapId === rid).length; const sData = { roadmapId: rid, title, desc: document.getElementById('stepDesc').value, date: document.getElementById('stepDate').value, order }; const tempId = Date.now(); sData.id = tempId; sData.completed = false; STATE.steps.push(sData); closeModal('stepModal'); renderRoadmapDetail(STATE.roadmaps.find(r => r.id === rid)); renderRoadmaps(); save(); toast('Step added ðŸ'); ofetch('add_step.php', sData, d => { const s = STATE.steps.find(x => x.id === tempId); if (s) s.id = d.id; renderRoadmapDetail(STATE.roadmaps.find(r => r.id === rid)); save(); }); }
...
function deleteStep(e, id, rid) { if (e) e.stopPropagation(); if (!confirm('Are you sure you want to delete this milestone?')) return; STATE.steps = STATE.steps.filter(s => s.id !== id); renderRoadmapDetail(STATE.roadmaps.find(r => r.id === rid)); renderRoadmaps(); save(); toast('Step deleted! ðŸ—‘ï¸'); ofetch('delete_step.php', { id }); }
...
function renderRoadmaps() { const el = document.getElementById('roadmapList'); if (STATE.roadmaps.length === 0) return el.innerHTML = '<div class="empty-state"><div class="empty-icon">ðŸ—ºï¸</div><p>Create your first roadmap</p></div>'; el.innerHTML = STATE.roadmaps.map(r => { const steps = STATE.steps.filter(s => s.roadmapId === r.id); const done = steps.filter(s => s.completed).length; const pct = steps.length ? Math.round(done / steps.length * 100) : 0; return `<div class="roadmap-item" onclick="openRoadmapDetail(${r.id})"><div style="display:flex;justify-content:space-between;align-items:flex-start"><div><div class="roadmap-title">${r.title}</div><div class="roadmap-desc">${r.desc || r.category}</div></div><span class="pill pill-accent">${r.category}</span></div><div class="roadmap-progress"><div class="roadmap-progress-fill" style="width:${pct}%;background:${r.color}"></div></div><div class="roadmap-meta"><span class="roadmap-steps-count">${done}/${steps.length} steps ${pct}%</span>${r.target ? `<span style="font-size:11px;color:var(--text3)">ðŸŽ¯ ${fmtDisplay(r.target)}</span>` : ''}</div></div>`; }).join(''); }
...
function renderRoadmapDetail(r) { const steps = STATE.steps.filter(s => s.roadmapId === r.id).sort((a, b) => a.order - b.order); const done = steps.filter(s => s.completed).length; const pct = steps.length ? Math.round(done / steps.length * 100) : 0; document.getElementById('roadmapDetail-view').innerHTML = `<div class="back-btn" onclick="navTo('roadmap')">â† Back to Roadmaps</div><div class="card" style="border-color:${r.color}20;background:${r.color}0a"><div style="display:flex;justify-content:space-between;align-items:start;margin-bottom:8px"><div><div style="font-family:'Syne',sans-serif;font-size:20px;font-weight:800">${r.title}</div><div style="font-size:13px;color:var(--text2);margin-top:2px">${r.desc || ''}</div></div><span class="pill pill-accent">${pct}%</span></div><div class="roadmap-progress" style="height:6px"><div class="roadmap-progress-fill" style="width:${pct}%;background:${r.color}"></div></div><div style="display:flex;justify-content:space-between;margin-top:8px"><span style="font-size:11px;color:var(--text2)">${done}/${steps.length} milestones</span>${r.target ? `<span style="font-size:11px;color:var(--text3)">Target: ${fmtDisplay(r.target)}</span>` : ''}</div></div><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px"><div class="section-title">Milestones</div><button class="btn-add" onclick="openStepModal(${r.id})">+ Step</button></div>${steps.length === 0 ? '<div class="empty-state"><div class="empty-icon">ðŸ</div><p>Add your first milestone</p></div>' : `<div class="step-timeline"><div class="step-line"></div>${steps.map((s, i) => `<div class="step-item"><div class="step-dot ${s.completed ? 'done' : i === done ? 'active' : ''}" onclick="toggleStep(event, ${s.id},${r.id})"></div><div class="step-content ${s.completed ? 'done' : ''}"><div style="display:flex;justify-content:space-between"><div class="step-content-title">${s.title}</div><span onclick="deleteStep(event, ${s.id},${r.id})" style="font-size:16px;color:var(--text3);cursor:pointer;padding:4px;">ðŸ—‘</span></div>${s.desc ? `<div class="step-content-desc">${s.desc}</div>` : ''}${s.date ? `<div style="font-size:10px;color:var(--text3);margin-top:4px">ðŸ“… ${fmtDisplay(s.date)}</div>` : ''}</div></div>`).join('')}</div>`}<div style="margin-top:16px"><button onclick="deleteRoadmap(event, ${r.id})" style="width:100%;padding:12px;border-radius:var(--radius-sm);border:1px solid rgba(245,100,124,.3);background:rgba(245,100,124,.08);color:var(--red);font-size:13px;cursor:pointer">Delete Roadmap</button></div>`; }
function openStepModal(roadmapId) { const r = STATE.roadmaps.find(r => r.id === roadmapId); if (navigator.onLine && r && r.pendingSync) return toast('â³ Waiting for cloud sync. Try again in a second!'); document.getElementById('stepTitle').value = ''; document.getElementById('stepDesc').value = ''; document.getElementById('stepDate').value = ''; document.getElementById('stepModal').dataset.roadmapId = roadmapId; document.getElementById('stepModal').classList.add('open'); }
...
function goToAttToday() { const today = new Date(); attCurrentDate = today; STATE.attSelectedDate = fmtDate(today); document.getElementById('attGoToDateInput').value = ''; renderAttCalendar(); renderAttendance(); toast('Jumped to today ðŸ“…'); }
...
function renderAttendance() { const el = document.getElementById('attendanceList'); if (!el) return; if (!STATE.attSelectedDate) STATE.attSelectedDate = fmtDate(new Date()); const selDateObj = new Date(STATE.attSelectedDate + 'T00:00:00'); const dayOfWeek = selDateObj.getDay(); const todaysRoutines = STATE.attendanceRoutines.filter(r => parseInt(r.dayOfWeek) === dayOfWeek).sort((a, b) => (a.startTime || a.time) > (b.startTime || b.time) ? 1 : -1); let html = `<div class="section-header" style="margin-top:20px;"><div class="section-title">Classes for ${selDateObj.toLocaleDateString('en', { weekday: 'short', month: 'short', day: 'numeric' })}</div></div>`; if (todaysRoutines.length === 0) { html += '<div class="empty-state"><div class="empty-icon">ðŸ–ï¸</div><p>No classes scheduled for today.</p></div>'; } else { html += todaysRoutines.map(r => { const log = STATE.attendanceLogs.find(l => l.routineId === r.id && l.date === STATE.attSelectedDate); const currentStatus = log ? log.status : null; const stats = calculateAttendanceStats(r.id); const timeStr = r.startTime && r.endTime ? `${formatTime(r.startTime)} - ${formatTime(r.endTime)}` : formatTime(r.time || '00:00'); return `<div class="att-card" style="border-left: 4px solid var(--accent); position: relative;"><div style="position: absolute; top: 16px; right: 16px; display: flex; gap: 8px;"><span onclick="openAttRoutineModalById(${r.id})" style="font-size:16px; color:var(--text3); cursor:pointer;" title="Edit Class">âœï¸</span><span onclick="deleteAttRoutine(event, ${r.id})" style="font-size:16px; color:var(--text3); cursor:pointer;" title="Delete Class">ðŸ—‘</span></div><div class="att-header" style="padding-right: 50px;"><div class="att-title">${r.subject}</div></div><div style="font-size:12px; color:var(--text2); margin-top:4px;">â° ${timeStr} &nbsp;|&nbsp; ðŸ« Room: ${r.room || 'N/A'}</div><div style="display:flex; justify-content:space-between; margin-top:12px; margin-bottom:8px; font-size:11px; color:var(--text2); font-family: 'Syne', sans-serif;"><span><b>Total Classes:</b> <span style="color:var(--text); font-size: 13px;">${stats.total}</span></span><span><b>Attended:</b> <span style="color:var(--green); font-size: 13px;">${stats.attended}</span></span><span><b>Missed:</b> <span style="color:var(--red); font-size: 13px;">${stats.missed}</span></span></div><div class="att-bar-bg" style="height: 4px;"><div class="att-bar-fill" style="width:${stats.pct}%; background: ${stats.pct >= 75 ? 'var(--green)' : 'var(--red)'}"></div></div><div style="display: flex; justify-content: space-between; font-size:11px; color:var(--text3); margin-bottom:12px;"><span>${stats.statsMsg}</span><span style="font-weight: 700; color: ${stats.pct >= 75 ? 'var(--green)' : 'var(--red)'};">${stats.pct.toFixed(1)}%</span></div><div class="att-controls" style="display:flex; gap:8px;"><button class="btn-secondary" style="flex:1; padding:8px; font-size:12px; background: ${currentStatus === 'attended' ? 'var(--green)' : 'var(--surface2)'}; color: ${currentStatus === 'attended' ? '#000' : 'var(--text)'};" onclick="updateAttLog(${r.id}, 'attended')">âœ… Attended</button><button class="btn-secondary" style="flex:1; padding:8px; font-size:12px; background: ${currentStatus === 'missed' ? 'var(--red)' : 'var(--surface2)'}; color: ${currentStatus === 'missed' ? '#fff' : 'var(--text)'};" onclick="updateAttLog(${r.id}, 'missed')">âŒ Missed</button><button class="btn-secondary" style="flex:1; padding:8px; font-size:12px; background: ${currentStatus === 'cancelled' ? 'var(--accent3)' : 'var(--surface2)'}; color: ${currentStatus === 'cancelled' ? '#000' : 'var(--text)'};" onclick="updateAttLog(${r.id}, 'cancelled')">â¸ï¸ Cancel</button></div></div>`; }).join(''); } el.innerHTML = html; }

// ============================================================
// ACADEMIC
// ============================================================
function saveAcademic() { const subject = document.getElementById('acadSubject').value.trim(); if (!subject) return toast('Need a subject!'); const editId = document.getElementById('academicModal').dataset.editId; const aData = { subject, type: document.getElementById('acadType').value, date: document.getElementById('acadDate').value, topic: document.getElementById('acadTopic').value, desc: document.getElementById('acadDesc').value, note: document.getElementById('acadNote').value }; if (editId) { aData.id = parseInt(editId); const idx = STATE.academic.findIndex(x => x.id === aData.id); if (idx >= 0) STATE.academic[idx] = aData; renderCalendar(); renderAcademic(); renderDashboard(); renderPlanner(); closeModal('academicModal'); save(); toast('Updated! ðŸŽ“'); ofetch('update_academic.php', aData); } else { const tempId = Date.now(); aData.id = tempId; STATE.academic.push(aData); renderCalendar(); renderAcademic(); renderDashboard(); renderPlanner(); closeModal('academicModal'); save(); toast('Saved! ðŸŽ“'); ofetch('add_academic.php', aData, d => { const a = STATE.academic.find(x => x.id === tempId); if (a) a.id = d.id; renderCalendar(); renderAcademic(); renderDashboard(); renderPlanner(); save(); }); } }
...
function renderAcademic() { const el = document.getElementById('academicList'); if (!STATE.academic.length) return el.innerHTML = '<div class="empty-state"><div class="empty-icon">ðŸŽ“</div><p>No upcoming exams or projects</p></div>'; const sorted = [...STATE.academic].sort((a, b) => new Date(a.date || '9999') - new Date(b.date || '9999')); el.innerHTML = sorted.map(a => `<div class="acad-card" onclick="openAcademicModalById(${a.id})"><div style="display:flex; justify-content:space-between; align-items:center;"><div class="acad-type">${a.type}</div><span onclick="delAcademic(event, ${a.id})" style="color:var(--text3); font-size:16px; cursor:pointer; padding:4px;">ðŸ—‘</span></div><div class="acad-title">${a.subject}</div>${a.date ? `<div class="acad-detail">ðŸ“… <b>Date:</b> ${new Date(a.date).toLocaleDateString('en', { weekday: 'short', month: 'short', day: 'numeric' })}</div>` : ''}${a.topic ? `<div class="acad-detail">ðŸŽ¯ <b>Topic:</b> ${a.topic}</div>` : ''}${a.desc ? `<div class="acad-detail">ðŸ“ <b>Desc:</b> ${a.desc}</div>` : ''}${a.note ? `<div class="acad-detail" style="margin-top:6px; font-style:italic; border-top:1px solid var(--border); padding-top:6px">Note: ${a.note}</div>` : ''}</div>`).join(''); }
    ...
    if (listEl) {
        listEl.innerHTML = noteChecklistItems.map((it, i) => `
                <div class="checklist-row">
                    ...
                    <span class="remove-x" onclick="removeNoteChecklistItem(${i})">âœ•</span>
                </div>`).join('');
    }
    const modeBtn = document.getElementById('noteModeBtn');
    if (modeBtn) modeBtn.textContent = noteChecklistMode ? 'ðŸ“ Switch to Text' : 'â˜‘ï¸ Switch to Checklist';
}
...
function toggleNoteArchive(e, id) { e.stopPropagation(); const n = STATE.notes.find(x => x.id === id); if (!n) return; n.archived = !n.archived; n.pinned = false; renderNotes(); save(); toast(n.archived ? 'Note archived ðŸ—„' : 'Note unarchived'); ofetch('update_note.php', { id, archived: n.archived, pinned: n.pinned }); }
function toggleNoteTrash(e, id) { e.stopPropagation(); const n = STATE.notes.find(x => x.id === id); if (!n) return; n.trashed = !n.trashed; n.pinned = false; n.archived = false; renderNotes(); save(); toast(n.trashed ? 'Moved to trash ðŸ—‘' : 'Note restored â™»ï¸'); ofetch('update_note.php', { id, trashed: n.trashed, pinned: n.pinned, archived: n.archived }); }
...
function renderNotes() {
    ...
    if (tagsFilterEl) {
        ...
        Array.from(allTags).sort().forEach(tag => {
            tagsHtml += `<div class="filter-tab ${activeNoteTag === tag ? 'active' : ''}" onclick="setNoteTagFilter('${tag}')" style="display:flex; align-items:center; gap:6px;">
                        #${escapeHtml(tag)}
                        <span style="opacity:0.5; font-size:14px; line-height:1;" onclick="deleteNoteTag(event, '${tag}')">âœ•</span>
                    </div>`;
        });
    ...
    if (!list.length) {
        ...
        el.innerHTML = `<div class="empty-state"><div class="empty-icon">ðŸ“</div><p>${msg}</p></div>`;
        return;
    }
    ...
    el.innerHTML = list.map(n => {
        if (n.isWhiteboard) {
            const actions = n.trashed ? `<span onclick="toggleNoteTrash(event,${n.id})" title="Restore">â™»ï¸</span><span onclick="deleteNoteForever(event,${n.id})" title="Delete forever">ðŸ—‘</span>` : `<span onclick="toggleNotePin(event,${n.id})" title="Pin">${n.pinned ? 'ðŸ“Œ' : 'ðŸ“'}</span><span onclick="toggleNoteArchive(event,${n.id})" title="Archive">ðŸ—„</span><span onclick="toggleNoteTrash(event,${n.id})" title="Trash">ðŸ—‘</span>`;
            return `<div class="note-card" style="border-left:4px solid var(--accent); background:var(--surface2);" onclick="${n.trashed ? '' : `if(typeof openWhiteboardModal === 'function') openWhiteboardModal(${n.id})`}">
                        ${n.pinned ? '<div class="note-pin-badge">ðŸ“Œ</div>' : ''}
                        <div class="note-title" style="color:var(--accent2);">ðŸŽ¨ ${escapeHtml(n.title)}</div>
...
function saveSleepLog() { const date = document.getElementById('sleepDate').value; const bedtime = document.getElementById('sleepBed').value; const wake = document.getElementById('sleepWake').value; if (!date || !bedtime || !wake) return toast('Fill in all fields!'); const durationMins = computeSleepDuration(bedtime, wake); const tempId = Date.now(); const sData = { id: tempId, date, bedtime, wake, durationMins }; STATE.sleepLogs.push(sData); renderSleep(); closeModal('sleepModal'); save(); toast('Sleep logged ðŸ˜´'); ofetch('add_sleep.php', sData, d => { const s = STATE.sleepLogs.find(x => x.id === tempId); if (s) s.id = d.id; renderSleep(); save(); }); }
...
function renderSleepChart(containerId, entries, maxHours) { maxHours = maxHours || 12; const el = document.getElementById(containerId); if (!el) return; if (!entries.length) { el.innerHTML = '<div class="empty-state" style="padding:16px 0"><p style="font-size:13px">No data yet</p></div>'; return; } const barW = entries.length > 14 ? '3px' : '10%'; el.innerHTML = `<div class="sleep-chart"><div class="sleep-ideal-line" style="bottom:${Math.min(95, (8 / maxHours) * 100)}%"></div>${entries.map(e => { const pct = Math.min(100, (e.hours / maxHours) * 100); const color = e.tag === 'good' ? 'var(--accent2)' : e.tag === 'low' ? 'var(--accent4)' : 'var(--accent3)'; return `<div class="sleep-bar-col" title="${e.date}: ${e.hours.toFixed(1)}h â€” ${e.label}"><div class="sleep-bar" style="height:${pct}%; background:${color}; width:${barW}"></div>${entries.length <= 14 ? `<div class="sleep-bar-label">${e.dateLabel}</div>` : ''}</div>`; }).join('')}</div>`; }
function getAvgTime(timeArray) { if (!timeArray.length) return 'â€”'; let totalMins = 0; timeArray.forEach(t => { const [h, m] = t.split(':').map(Number); let shiftedH = h < 12 ? h + 24 : h; totalMins += (shiftedH * 60) + m; }); let avgMins = Math.round(totalMins / timeArray.length) % 1440; const h = Math.floor(avgMins / 60) % 24; const m = avgMins % 60; const ampm = h >= 12 ? 'PM' : 'AM'; const dispH = h % 12 === 0 ? 12 : h % 12; return `${dispH}:${String(m).padStart(2, '0')} ${ampm}`; }
function renderSleep() { if (!document.getElementById('sleepMeterCard')) return; const sorted = [...STATE.sleepLogs].sort((a, b) => a.date.localeCompare(b.date)); const withQuality = sorted.map(e => ({ ...e, ...sleepQuality(e.durationMins, e.bedtime), dateLabel: fmtDisplay(e.date) })); const last7 = withQuality.slice(-7); const last30 = withQuality.slice(-30); const avg = arr => arr.length ? (arr.reduce((s, e) => s + e.hours, 0) / arr.length) : 0; const latest = withQuality[withQuality.length - 1]; const meterEl = document.getElementById('sleepMeterCard'); if (!latest) { meterEl.innerHTML = '<div class="empty-state"><div class="empty-icon">ðŸ˜´</div><p>Log your first night to see your sleep meter</p></div>'; } else { const color = latest.tag === 'good' ? 'var(--accent2)' : latest.tag === 'low' ? 'var(--accent4)' : 'var(--accent3)'; meterEl.innerHTML = `<div class="stat-card"><div style="font-size:13px;color:var(--text2)">Last logged night (${fmtDisplay(latest.date)})</div><div style="font-size:30px;font-weight:800;margin:6px 0;">${latest.hours.toFixed(1)}h</div><div style="color:${color};font-weight:700">${latest.label}</div><div style="font-size:12px;color:var(--text2);margin-top:6px">ðŸ› ${latest.bedtime} â†’ â° ${latest.wake}</div></div>`; } document.getElementById('sleepAvgWeek').textContent = last7.length ? avg(last7).toFixed(1) + 'h' : 'â€”'; document.getElementById('sleepAvgMonth').textContent = last30.length ? avg(last30).toFixed(1) + 'h' : 'â€”'; const bedtimes = last7.map(e => e.bedtime); const waketimes = last7.map(e => e.wake); if (document.getElementById('sleepAvgBed')) document.getElementById('sleepAvgBed').textContent = getAvgTime(bedtimes); if (document.getElementById('sleepAvgWake')) document.getElementById('sleepAvgWake').textContent = getAvgTime(waketimes); renderSleepChart('sleepWeekChart', last7, 12); renderSleepChart('sleepMonthChart', last30, 12); const listEl = document.getElementById('sleepList'); if (listEl) { if (!withQuality.length) { listEl.innerHTML = '<div class="empty-state"><div class="empty-icon">ðŸ˜´</div><p>No sleep logs yet</p></div>'; } else { const color = e => e.tag === 'good' ? 'var(--accent2)' : e.tag === 'low' ? 'var(--accent4)' : 'var(--accent3)'; listEl.innerHTML = [...withQuality].reverse().map(e => `<div class="card" style="display:flex;justify-content:space-between;align-items:center"><div><div style="font-weight:700">${fmtDisplay(e.date)}</div><div style="font-size:12px;color:var(--text2)">ðŸ› ${e.bedtime} â†’ â° ${e.wake} Â· ${e.hours.toFixed(1)}h</div><div style="font-size:12px;color:${color(e)}">${e.label}</div></div><span onclick="deleteSleepLog(${e.id})" style="color:var(--text3);cursor:pointer;font-size:16px">ðŸ—‘</span></div>`).join(''); } } }

// ============================================================
// EXPORT / IMPORT
// ============================================================
function exportData() { const localData = { ...STATE }; delete localData.syncQueue; const uri = 'data:text/json;charset=utf-8,' + encodeURIComponent(JSON.stringify(localData)); const a = document.createElement('a'); a.href = uri; a.download = 'lifeeasy_supabase_backup.json'; document.body.appendChild(a); a.click(); a.remove(); toast('Backup Exported Locally! ðŸ’¾'); }
function importData(event) { const file = event.target.files[0]; if (!file) return; toast('Reading backup file...'); const reader = new FileReader(); reader.onload = async e => { try { const jsonData = JSON.parse(e.target.result); Object.assign(STATE, jsonData); save(); toast('Local cache restored! Uploading modifications...'); setTimeout(() => location.reload(), 1500); } catch { toast('Invalid backup file! âŒ'); } }; reader.readAsText(file); event.target.value = ''; }
...
function renderNavSettings() {
    ...
    STATE.navPreferences.forEach((key, index) => {
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; background:var(--surface2); padding:10px 14px; margin-bottom:8px; border-radius:10px; border:1px solid var(--border);">
            ...
            <div style="display:flex; gap:6px;">
                <button class="btn-secondary" style="padding:6px 10px; font-size:14px;" onclick="moveNavModule(${index}, -1)" ${index === 0 ? 'disabled style="opacity:0.3"' : ''}>â–²</button>
                <button class="btn-secondary" style="padding:6px 10px; font-size:14px;" onclick="moveNavModule(${index}, 1)" ${index === STATE.navPreferences.length - 1 ? 'disabled style="opacity:0.3"' : ''}>â–¼</button>
...
async function buildRoutineGrid(pdf) {
    ...
    for (let p = 1; p <= pdf.numPages; p++) {
        ...
        // 1. Find the "Slot N" header items â€” used only as a ROUGH first pass, because
        ...
        // then take each column's MINIMUM x from that row as its true left edge â€” time text
        ...
        Object.keys(buckets).forEach(key => {
            ...
            // a fraction of a point past a column border â€” skip it rather than create a
...
async function handleRoutineUpload(event) {
    ...
    toast('Reading PDF... Please wait â³');
    ...
    try {
        ...
        fileReader.onload = async function () {
            ...
            if (cells.length > 0) {
                ...
                toast('Processing with AI (fallback mode)... ðŸ¤–');
                await sendRawTextToAI(fullText);
            }
        };
        fileReader.readAsArrayBuffer(file);
    } catch (error) {
        toast('Failed to read PDF! âŒ');
        console.error(error);
    }
...
async function sendCellsToAI(cells) {
    toast('Reading course names with AI... ðŸ¤–');
    ...
    try {
        ...
        toast('Failed to process routine! âŒ');
...
function saveExtractedRoutines(classes) {
    ...
    classes.forEach(cls => {
        ...
        // à¦²à§‹à¦•à¦¾à¦² à¦¸à§à¦Ÿà§‡à¦Ÿà§‡ à¦ªà§à¦¶ à¦•à¦°à¦¾
        STATE.attendanceRoutines.push(rData);
        addedCount++;

        // à¦¬à§à¦¯à¦¾à¦•à¦—à§à¦°à¦¾à¦‰à¦¨à§à¦¡à§‡ Supabase-à¦ à¦¸à¦¿à¦™à§à¦• à¦•à¦°à¦¾
    ...
    renderAttCalendar();
    renderAttendance();
    toast(`Successfully added ${addedCount} classes! ðŸŽ‰`);
}
