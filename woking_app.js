        ...
        try {
            ...
            if (d && d.success) {
                ...
                if (item.endpoint.startsWith('add_') && d.id) {
                    ...
                    idMap[tempId] = d.id;
                    for (const key of ['tasks', 'plans', 'counters', 'money', 'alarms', 'roadmaps', 'steps', 'attendance', 'academic', 'accounts', 'expenses']) {
                        const arr = STATE[key];
...
async function load() {
    try {
        ...
        if (rNotes.data) STATE.notes = rNotes.data.map(n => ({ ...n, updatedAt: n.updated_at }));
